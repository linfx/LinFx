...
namespace LinFx.Domain.Uow
{
    ...
    public abstract class UnitOfWorkBase : IUnitOfWork
	{
		...
		public virtual IDisposable SetTenantId(int? tenantId, bool switchMustHaveTenantEnableDisable)
		{
			...
			if(switchMustHaveTenantEnableDisable)
			{
				mustHaveTenantEnableChange = tenantId == null
					? DisableFilter(DataFilters.MustHaveTenant)
					: EnableFilter(DataFilters.MustHaveTenant);
			}
			else
			{
		...
		private void PreventMultipleBegin()
		{
			...
			{
				throw new LinFxException("This unit of work has started before. Can not call Start method more than once.");
			}
		...
		private DataFilterConfiguration GetFilter(string filterName)
		{
			...
			if(filter == null)
			{
				throw new LinFxException("Unknown filter name: " + filterName + ". Be sure this filter is registered before.");
			}
